...
void create_pairs() {
    ...
    pid_t hot_pids[MAX_TASKS], cold_pids[MAX_TASKS];
    ...
    num_pairs = (num_hot < num_cold) ? num_hot : num_cold;
    if (num_pairs > MAX_CORES) num_pairs = MAX_CORES;

    printf("Creating %d pairs on %d cores:\n", num_pairs, num_pairs);
    for (int i = 0; i < num_pairs; i++) {
        ...
        pin_to_core(cores[i].hot_pid, i);
        pin_to_core(cores[i].cold_pid, i);
        kill(cores[i].cold_pid, SIGSTOP);

        printf("  Core %d: HOT=%d, COLD=%d\n", i, cores[i].hot_pid, cores[i].cold_pid);
    }
    printf("\n");
}
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        ...
        printf("Example: Start 4 int_power and 4 io_power, then:\n");
        printf("  %s $(pgrep int_power) $(pgrep io_power)\n", argv[0]);
        return 1;
    }
    ...
    printf("Thermal Multi-Task Controller\n");
    printf("Managing %d tasks\n\n", num_tasks);
